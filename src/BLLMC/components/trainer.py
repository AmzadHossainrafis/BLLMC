...
class Trainer(ABC):
    ...
    def _setup_optimizer(self):
        if self.config.optimizer == "AdamW":
            ...
            raise NotImplementedError(f"Optimizer {self.config.optimizer} not supported.")

    def _save_checkpoint(self, epoch: int, loss: float):
        os.makedirs(self.config.checkpoint_dir, exist_ok=True)
        checkpoint_path = os.path.join(self.config.checkpoint_dir, f"ckpt_epoch_{epoch}.pt")
        torch.save({
            'epoch': epoch,
...
class LLMTrainer(Trainer):
    ...
    def train_step(self, inputs: torch.Tensor, targets: torch.Tensor):
        ...
        loss = self.criterion(model_output.view(-1 , model_output.size(-1)) , targets.view(-1))
        loss.backward()
        ...
        for epoch in range(self.config.max_epochs):
            ...
            for batch_idx, (inputs, targets) in pbar:
                ...
                if batch_idx % self.config.gen_indx == 0:
                    ...
                    start_tokens = tokenizer.encode(prompt, allowed_special={"<|endoftext|>"})
                    x = (torch.tensor(start_tokens, dtype=torch.long, device=self.device)[None, :])  
