...
class Llama3Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, use_cache=False):
        """
        Forward pass for a Llama3 block.
        This implementation uses pre-normalization as is standard in Llama architectures.

        Args:
            x (torch.Tensor): Input tensor of shape (Batch, Seq_len, Emb_dim).
            use_cache (bool): If True, enable KV caching for autoregressive generation.
